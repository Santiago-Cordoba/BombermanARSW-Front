...
export type PowerUpType = 'BOMB' | 'FIRE' | 'SPEED' | 'LIFE_UP' | 'INVINCIBILITY' | 'FIRE_PLUS';
...
type Player = {
  ...
  bombRange: number;
  lives: number;
  invincible: boolean; // ✅ ← Esta línea es la clave
};
...
const GamePage: React.FC = () => {
  ...
  const [localPowerUps, setLocalPowerUps] = useState<{x: number, y: number, type: PowerUpType}[]>([]);
  ...
  const defaultMap = useMemo<GameMap>(() => ({
    ...
    cells: Array(13).fill(null).map((_, y) => 
      Array(15).fill(null).map((_, x) => ({
        ...
        powerUpType: ['BOMB', 'FIRE', 'SPEED', 'INVINCIBILITY', 'LIFE_UP', 'FIRE_PLUS'][Math.floor(Math.random() * 6)] as PowerUpType
      }))
    )
  }), []);
    ...
    const subscription = subscribe<GameMessage>(
      ...
      (message) => {
        ...
        if (message.type === 'GAME_START') {
          ...
          if (message.powerUps) {
            const allowedTypes: PowerUpType[] = ['BOMB', 'FIRE', 'SPEED', 'INVINCIBILITY', 'LIFE_UP', 'FIRE_PLUS'];
            setLocalPowerUps(
    ...
    <div className="game-container" ref={gameContainerRef} tabIndex={0}>
                      ...
                      >
                        ...
                        {powerUp && !bomb && !explosion && (
                          <div className={`game-power-up ${powerUp.type.toLowerCase()}`}>
                            {powerUp.type === 'FIRE_PLUS' && <span className="game-power-up fire-plus">🔥+</span>}
                            {powerUp.type === 'BOMB' && 'B+'}
                            {powerUp.type === 'FIRE' && 'F+'}
                            {powerUp.type === 'SPEED' && 'S+'}
                            {powerUp.type === 'INVINCIBILITY' && <span className="game-power-up invincibility">🛡️</span>}
                            {powerUp.type === 'LIFE_UP' && '❤️'}
                          </div>
                        )}
      ...
      <div className="game-controls">
        ...
        <div className="power-up-legend">
          ...
          <div className="legend-item">
            <span className="power-up-icon shield">🛡️</span> = Invencibilidad temporal
          </div>
          <div className="legend-item">
            <span className="power-up-icon fire-plus">🔥+</span> = Explosión +1 rango adicional
          </div>
        </div>
      </div>
    </div>
